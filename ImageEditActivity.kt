...
import PdfCreator.Editor.databinding.ActivityImageEditBinding
...
import com.PdfCreator.Editor.AndroidUtils
import com.PdfCreator.Editor.AndroidUtils.filter_bw
import com.PdfCreator.Editor.AndroidUtils.filter_clr1
import com.PdfCreator.Editor.AndroidUtils.filter_clr2
import com.PdfCreator.Editor.AndroidUtils.filter_dark
import com.PdfCreator.Editor.AndroidUtils.filter_duo
import com.PdfCreator.Editor.AndroidUtils.filter_elegant
import com.PdfCreator.Editor.AndroidUtils.filter_euro
import com.PdfCreator.Editor.AndroidUtils.filter_film
import com.PdfCreator.Editor.AndroidUtils.filter_fresh
import com.PdfCreator.Editor.AndroidUtils.filter_golden
import com.PdfCreator.Editor.AndroidUtils.filter_ins
import com.PdfCreator.Editor.AndroidUtils.filter_lomo
import com.PdfCreator.Editor.AndroidUtils.filter_movie
import com.PdfCreator.Editor.AndroidUtils.filter_pink
import com.PdfCreator.Editor.AndroidUtils.filter_retro
import com.PdfCreator.Editor.AndroidUtils.filter_tint
import com.PdfCreator.Editor.GPUImageFilterTools
import com.PdfCreator.Editor.adapter.*
import com.PdfCreator.Editor.ads.AdInterstitialGoogle
import com.PdfCreator.Editor.model.EffectData
import com.PdfCreator.Editor.model.FilterData
import com.PdfCreator.Editor.utils.FileExternalPath.getRealPathFromURI
...
class ImageEditActivity : AppCompatActivity(), View.OnClickListener,
    ...
    lateinit var viewBinding: ActivityImageEditBinding
    var imageId1: String? = null
    var orgFileName: String? = null
    var imageId2: Int? = null
    var mFlag1: Boolean = false
    ...
    override fun onClick(v: View?) {
        when (v!!.id) {
            ...
            R.id.done -> {
                ...
                getByteArray()
                intent.putExtra("EditedImage", byteArray)
                intent.putExtra("id_image", imageId2)
                intent.putExtra("orgFileName", orgFileName)
                intent.putExtra("flag1", mFlag1)
                intent.putExtra("editing", getEditing)
                intent.putExtra("clickedImageUri", sentUri)
                intent.putExtra("foldersId",backPressedId1)
                intent.putExtra("folderName",folderName)
                intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION)
                Log.e("TAG", "onClick: $getEditing", )
...
//            overlay_light.setImageBitmap((overlay_light.drawable as BitmapDrawable).bitmap)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewBinding = ActivityImageEditBinding.inflate(layoutInflater)
        setContentView(viewBinding.root)

        loadLInterstitialAd()
        imageId1 = intent.getStringExtra("id_image")
        orgFileName = intent.getStringExtra("orgFileName")
        imageId2 = imageId1!!.toInt()
        sentUri = intent.getStringExtra("clickedImageUri")
        backPressedId1 = intent.getStringExtra("foldersId")

        // Toast.makeText(this, "getting in Image edit"+imageId2.toString(), Toast.LENGTH_SHORT).show()

        multipleFunctions()

        viewBinding.myShimmer.ivBack.setOnClickListener {
            onBackPressed()
        }
        ...
        mFlag1 = intent.getBooleanExtra("flag1", false)
        getEditing = intent.getBooleanExtra("editing", false)
        Log.e("TAG", "oncreate: $getEditing", )

        val myURI = Uri.encode(imageUri).toUri()

        val path = getRealPathFromURI(myURI)
        ...
        original_bitmap = BitmapFactory.decodeFile(path)
        ...
        img_main.setImageBitmap(image_bitmap)
    ...
    private fun multipleFunctions() {
        val toolbarMenu: ImageView = findViewById(R.id.toolbarIconMenu)
        val toolbarPdfIcon: ImageView = findViewById(R.id.showPdfIcon)
        var toolbarHeading: TextView = findViewById(R.id.toolbar_title)

        folderName = intent.getStringExtra("folderName")
        toolbarMenu.visibility = View.GONE
        toolbarPdfIcon.visibility = View.GONE
    ...
    inner class FilterDetailAdapter(filters: Array<FilterData>) :
        ...
        override fun onBindViewHolder(holder: FilterDetailHolder, position: Int) {
            ...
            holder.thumbnail_filter.setImageResource(R.drawable.dummy_image)
    ...
    inner class OptionAdapter : RecyclerView.Adapter<OptionAdapter.OptionHolder>() {
        ...
        override fun onBindViewHolder(holder: OptionHolder, position: Int) {
            ...
            if (selectedIndex == position) {
                ...
                holder.img_option.setBackgroundResource(R.drawable.circle_selection)
            } else {
                holder.txt_option.setTextColor(resources.getColor(R.color.colorAccent))
                holder.img_option.setColorFilter(resources.getColor(R.color.colorAccent))
                holder.img_option.setBackgroundResource(0)
            }
        ...
        inner class OptionHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
            ...
            var ll_option: ConstraintLayout = itemView.findViewById(R.id.ll_option)
        }
    }
...
//                        )
    ...
    private fun getByteArray() {
        ...
        val bmap: Bitmap = img_main.getDrawingCache()
    ...
    override fun onBackPressed() {
        ...
        intent.putExtra("foldersId",backPressedId1)
        intent.putExtra("folderName",folderName)
        Log.e("TAG", "onBackPressed: $getEditing", )
        startActivity(intent)
